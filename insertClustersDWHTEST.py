...
df = pd.read_csv('ClustersDataSetClientsNewk2.csv')
...
cursor.execute("""
IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='DIM_CLUSTERS' and xtype='U')
CREATE TABLE DIM_CLUSTERS (
    ID_DIMCLUS INT IDENTITY(1,1) PRIMARY KEY,
    CODEASSU INT,
    CODEINTE INT,
    CODECATE INT,
    PRIMETOTALE FLOAT,
...
# Insérer  données dans la table SQL
for index, row in df.iterrows():
    cursor.execute("INSERT INTO DIM_CLUSTERS (CODEASSU, CODEINTE, CODECATE, PRIMETOTALE, ClusterID) VALUES (?, ?, ?, ?, ?)",
